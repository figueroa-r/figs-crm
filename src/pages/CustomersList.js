...
import {
  Avatar,
  Card, Checkbox, Divider, IconButton, MenuItem, Paper, Popover, Stack, Table, TableBody,
...
// here, we have the values for our different columns, including labels for our label component. An extra column is created
// for our menu on each table row... this inlcudes a menu to edit/delete a single row
...
// sorting functions for ascending/descending on our table
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_customer) => _customer.name.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
              ...
              <Table>
                ...
                <TableBody>
                  {filteredCustomers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, avatarUrl, name, status, companyType, isVerified } = row;
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedCustomer}>
                        ...
                        <TableCell align="left">
                          <Label color={(status === 'inactive' && 'error') || 'success'} variant='soft'>{sentenceCase(status)}</Label>
                        </TableCell>
